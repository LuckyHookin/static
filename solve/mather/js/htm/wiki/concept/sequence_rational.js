...
detail('有理数列',Table([ZLR('名称 定义 第n项 前n项 性质')],[
	...
	['伯努利数\nBernoulli',khrA([Eq([['B_n',sum('j',0,'n','(-1)^jC_{n+1}^{j+1}'+frac('n!','(n+j)!','')+sum('k',0,'j','(-1)^{j-k}C_j^kk^{n+j}','',''),'','')],
			frac('(-1)^j','j+1','')+sum('k',0,'j','(-1)^{j-k}C_j^kk^n','',''),
			frac('(-1)^k','k+1','')+sum('j',0,'k','(-1)^{k-j}C_k^jj^n','','')]),
			'B_n=n!'+kdet([[1,0,'⋯',0,1],['1\\/{2!}',1,'⋯',0,0],['⋮','⋮','','⋮','⋮'],['1\\/{n!}','1\\/{(n-1)!}','⋯',1,0],['1\\/{(n+1)!}','1\\/{n!}','⋯','1\\/{2!}',0]]),

			'偶数项B_{2p}=-\\frac{(2p)!}{2^{2p}-2}'+kdet([[1,0,0,'⋯',0,1],['1\\/{3!}',1,0,'⋯',0,0],['1\\/{5!}','1\\/{3!}',1,'⋯',0,0],['⋮','⋮','⋮','','⋮','⋮'],['1\\/{(2p+1)!}','1\\/{(2p-1)!}','1\\/{(2p-3)!}','⋯','1\\/{3!}',0]])
	...
	['九连环数列',kbrA(['解开n个环所需步数',frac('2^{n+1}-k',3,''),'其中k='+piece([[1,'当n是奇数时'],[2,'当n是偶数时']]),]),
		khrA([
