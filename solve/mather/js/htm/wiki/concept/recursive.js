...
wiki['Concept/Expression/Recursive']=
	detail('阿克曼函数Ackermann （双归纳，扩展基本算术运算+ * ^ ↑）',ksc('A(m,n)='+
		piece([
			['n+1','当m=0时'],
			['A(m-1, 1)','当m>0且n=0时'],
		...
		Kx(kul(['A(0,n)=n+1',
		'A(1,n)=2+(n+3)-3=n+2'+kbr+'A(1,n)=A(0, A(1,n-1))=A(0, A(0, A(1,n-2)))= ⋯ =A(1,0)+n=A(0,1)+n=2+n',
		'A(2,n)=2(n+3)-3=2n+3'+kbr+'A(2,n)=A(1, A(2,n-1))=2+A(2,n-1)= ⋯ =2n+A(2,0)=2n+3',
		'A(3,n)=2↑(n+3)-3=2^{n+3}-3'+kbr+'A(3,n)+3=A(2, A(3,n-1))+3=2A(3,n-1)+3+3=2(A(3,n-1)+3)= ⋯ =2^n(A(3,0)+3)=2^{n+3}',
		'A(4,n)=2↑↑(n+3)-3'+kbr+'A(4,n)+3=A(3, A(4,n-1))+3=2^{A(4,n-1)+3}= ⋯ ='+msups([2,2,'⋰',2,'A(4,0)+3'],1)+'='+msups([2,2,'⋰',2,2,2,2],1)+'=2↑↑(n+3)',
		'A(5,n)=2↑↑↑(n+3)-3',
		'A(6,n)=2↑↑↑↑(n+3)-3',
		'A(m,n)=2↑^{m-2}(n+3)-3',
		...
		'A(3,0)=A(2,1)=A(1, A(2,0))=A(1,3)=5=2^3-3=2↑3-3',
		'A(4,0)=A(3,1)=A(2, A(3,0))=A(2,5)=13='+msups([2,2,2],1)+'-3=2↑↑3-3',
		...
		'<div id=Ackermann>A('+num(1)+','+num(1)+') = <input type=text />','wiki');
