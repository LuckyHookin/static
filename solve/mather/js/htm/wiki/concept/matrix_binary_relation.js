...
detail('矩阵（二元关系）',Table([ZLR('名称 记法 充要条件 性质')],[
	ZLR('相似\nsimilar____A \\sim B____'+khrA(['AP=PB ⇔ P^{-1}AP=B \\\\ 其中P可逆',
		...
		kul(['相似 ⇒ '+piece(['等价关系','特征值相同','正定性相同 \\\\ 证明：上推','特征多项式相同','最小多项式相同（反之不成立）','秩相等','迹相等','行列式相等','伴随矩阵相似 \\\\ 证明：等式两边求伴随矩阵']),
			...
			['A、B=P^{-1}AP相似，A有特征向量x ⇒ B有特征向量P^{-1}x','证明：Ax=kx ⇒ PBP^{-1}x=kx ⇒  B(P^{-1}x)=k(P^{-1}x)'].join(kbr),
			['伴随矩阵相似 ⇏ 原矩阵相似','举例：A='+kxf('diag')+'(1,0,0)与B='+kxf('diag')+'(2,0,0),','伴随矩阵都是0,但A和B不相似'].join(kbr),
			piece(['A的特征值互异（无重根）','A是对称矩阵'],1)+'⇒ A与对角阵Λ相似（可对角化）',
			['二阶实矩阵满足A^2+E=0 ⇒ A与副对角阵'+kxf('cdiag')+'(-1,1)相似','证明：求出A具体形式，再尝试施行一个相似变换'].join(kbr),
			['A与对角阵Λ相似（可对角化）','⇒kA^n±E与对角阵(kΛ^n±E)相似（可对角化）'].join(kbr)
		]),'','____'),
