...
wiki['Concept/Number/Integer/Divide']=detail('整除的快速判别法 '+ksc('k|m=□10^n+□10^{n-1}+□10^{n-2}+⋯+□10^2+□10+□'),
		Table([['除数k','k|m的充要条件']],[
			[2,['10=5×2 即2|末位='+kmod('0,2,4,6,8',0,2)].join(hr)],
			[3,['10=3×3+1 即3|□+□+□+⋯+□+□+□',ol([
				ksc('模3剩余类[0]：'+kmod('0,3,6,9',0,3)+',[1]：'+kmod('1,4,7',1,3)+',[2]：'+kmod('2,5,8',2,3)),
				'系数□去掉剩余类[0]',
				'系数□成对去掉互余剩余类[1,2]',
			...
			[4,['100=25×4 即4|末2位数',
				...
				'相当于满足同余方程'+ksc(kmod('[x]2+[y]',0,4)),
				].join(hr)],
			[5,['10=2×5 即5|末位='+kmod('0,5',0,5)].join(hr)],
			[6,['6=2×3 即需同时满足2，3|m',
				ksc('10=6+4 即6|□4^n+□4^{n-1}+□4^{n-2}+⋯+□4^2+□4+□')+br+
				'也即2|末位 且3|□□□⋯□□□（根据4=3+1）',
			...
			[7,[ksc('1000=13×11×7-1，'+'10^n'+'=13×11×7×10^{n-3}-10^{n-3} 即7|-□□□⋯+□□□')+br+
				ksc(kmod('10^n','-10^{n-3}',7))+' 即从末位往前，每3位截取，并变号，7|计算结果',
				'100=14×7+2，10=7+3，即7|-□□□⋯+□2+□3+□',
				'也即 7|⋯+□2+□3+□-□2-□3-□+□2+□3+□',ol([
				ksc('模7剩余类[0]：'+kmod('0,7',0,7)+',[1]：'+kmod('1,8',1,7)+',[2]：'+kmod('2,9',2,7)+',[3],[4],[5],[6]'),
				'系数□去掉剩余类[0]：'+ksc(kmod('0,7',0,7)),
				...
				'相当于满足同余方程'+ksc(kmod('[x]2+[y]3+[z]',0,7)),
				
				'7|计算结果',

				]),ol([
					ksc(kfrac([1,7]))+'=0.142；857 余数分别是132；6（-1）45',
					'即7|⋯⋯+□□□□□□+□□□□□□ （每6位一截）',
			...
			[8,['1000=125×8 即8|末3位数□□□',
				...
				'相当于满足同余方程'+ksc(kmod('[x]4+[y]2+[z]',0,8)),
			

				].join(hr)],
			[9,['10=9+1 即9|□+□+□+⋯+□+□+□',ol([
				'模9剩余类[0]：'+ksc(kmod('0,9',0,9))+',[1],[2],⋯,[8]',
				'系数□去掉剩余类[0]',
			...
			[11,[ksc('10=11-1，'+'10^n'+'=11×10^{n-1}-10^{n-1} 即11|⋯+□-□+□'),
				ksc('1000=13×7×11-1，'+'10^n'+'=10^{n-3}(11×91-1)')+br+
				ksc(kmod('10^n','-10^{n-3}',11))+' 即从末位往前，每3位截取，并变号，11|计算结果',ol([
				'奇偶位数的系数□成对去掉相同数',
				'同奇同偶位，去掉11倍数个相同数',
				'奇位数之和，与偶位数之和，相减',
				'11|计算结果',
				]),ol([
					ksc(kfrac([1,11])+'=0.0；9 余数分别是1；10（-1）'),
					'即11|⋯⋯+□□+□□ （每2位一截）',
			...
			[12,['12=2×2×3 即需同时满足4，3|m',
				ksc('10=12-2 即12|□(-2)^n+□(-2)^{n-1}+□(-2)^{n-2}+⋯+□(-2)^2+□(-2)+□'),
				].join(hr)],


			[13,[ksc('1000=11×7×13-1，'+'10^n'+'=11×7×13×10^{n-3}-10^{n-3} 即13|-□□□⋯+□□□')+br+
				ksc(kmod('10^n','-10^{n-3}',13))+' 即从末位往前，每3位截取，并变号，13|计算结果',
				...
				'相当于满足同余方程'+ksc(kmod('[x](-4)+[y](-3)+[z]',0,13)),
				
				'13|计算结果',
				]),ol([
					ksc(kfrac([1,13]))+'=0.076；923 余数分别是1 10 9；12（-1） 3 4',
					'即13|⋯⋯+□□□□□□+□□□□□□ （每6位一截）',
			...
			[15,['15=3×5 即需同时满足3，5|m',
				...
				ksc(kmod('10^n',10,15))+' 其中n>0 即从末位往前，各位数求和然后乘以10，加上末位数，15|计算结果',

				'100=6×15+10, 1000=60×15+100, 10000=600×15+1000 即15|(□⋯□)10+□=m'+br+
				ksc(kmod('10^n','10^{n-1}',15))+' 其中n>1 此方法无实际用途',
			...
			[16,['10000=5^4×16 即16|末4位数□□□□',
				...
				'相当于满足同余方程'+ksc(kmod('[w]8+[x]4+[y]2+[z]',0,16)),
			...
			[17,['1000=59×17-3，10000=588×17+4, 100000=5882×17+6, 1000000=58823×17+9, 10000000=588235×17+5, 100000000=5882353×17-1'
					,ol([
					ksc(kfrac([1,17]))+'=0.05882352；94117647 余数分别是1 10 15 14 4 6 9 5；16（-1） 7 2 3 13 11 8 12',
					'即17|⋯⋯+□⋯□+□⋯□ （每16位一截）',
			...
			[19,[ol([
					ksc(kfrac([1,19]))+'=0.052631578；947368421 余数分别是1 10 5 12 6 3 11 15 17；18（-1） 9 14 7 13 16 8 4 2',
					'即19|⋯⋯+□⋯□+□⋯□ （每18位一截）',
					'系数去掉，19倍数个同位相同数',
					'同位分别求和再求余',
				])].join(hr)],
			...
			[23,[ol([
					ksc(kfrac([1,23]))+'=0.04347826086；95652173913 余数分别是1 10 8 11 18 19 6 14 2 16；22（-1） 13 15 12 5 4 17 9 21 7',
					'即23|⋯⋯+□⋯□+□⋯□ （每22位一截）',
					'系数去掉，23倍数个同位相同数',
					'同位分别求和再求余',
				])].join(hr)],
			...
			[647,[ol([
					ksc(kfrac([1,647]))+'=0.001545595054095826893353941267387944358578052550231839258 '+br+
			...
			['素数p',[ol([
					ksc(kfrac([1,'p']))+'=0.0⋯ 余数分别是1 10 ⋯ t ⋯ p-1（-1）⋯ p-t（-t） ⋯'+br+
					'余数可以只算到1半（最多'+ksc(kfrac(['p-1',2]))+'位），根据模p，写出另一半，减小计算量'+br+'原理参见：wikipedia'+
					ol(['设循环节是'+ksc(kxo('xy','-'))+'（x,y各占一半数字，长度都是l≤'+ksc(kfrac(['p-1',2]))+'），p=2d+1',
					'根据等比求和公式，'+ksc(kfrac(['x10^{-l}+y10^{-2l}','1-10^{-2l}'])+' = '+kfrac([1,'p'])),
					ksc('(x10^l+y)p = 10^{2l}-1 = (10^l+1)(10^l-1)'),
					ksc('(x+y)p+(10^l-1)xp = (10^l+1)(10^l-1)'),
					...
					'用矩阵表示，余数向量A（1 10 ⋯ t ⋯ p-1（-1）⋯ p-t（-t） ⋯）, 系数向量B（个，十，百，千⋯位），'+ksc(kmod('AB^T',0,'p','','','')),
				]),
					ol(['设p位数是t，m从个位往前，每t位数，求模k同余',
					ksc(kmod('10^{st}','x^s','p','','','')+' 其中'+kmod('x','10^t','p','','','')),
					'用矩阵表示，余数向量A（1 10 ⋯ t ⋯ p-1（-1）⋯ p-t（-t） ⋯）, 系数向量B（个，十，百，千⋯位），'+ksc(kmod('AB^T',0,'p','','','')),
				]),
					'若p位数过亿时，'].join(hr)],
		...
		+detail('常用整除判断法',Table([['数m','m|n ⇔ m|n的下列值']],[
			...
			['16、625','末四位数'],
			['$2^n$、$5^n$','末n位数'],
			['7、11、13',['末三位之前的数-末三位数','【大割尾法】a*(7*11*13-1)+b → a-b','推论：原数字后面接偶数个3位数字串，不改变整除性'].join(kbr)],
		...
		+detail('常用素数整除判断法',Table([['素数p','p|n ⇔ p|n的下列值']],[
			...
			[[kxo('st','-'),'其中(s,t)=1','t是个位数'].join(kbr),[
					ksc([
					'【割尾法1】*s = a*('+kxo('st','-')+'-t)+b*s → ta-sb',
					'特别地, s=1时，'+kxo('1t','-')+' → ta-b',
					'特别地, t=1时，'+kxo('s1','-')+' → a-sb',

					'特别地, s=t=1时，11 → a-b',
					'特别地, t=3时，'+kxo('s3','-')+' → 3a-sb',
					'特别地, t>5时，ta-sb → (10-t)a+(s+1)b',
					'特别地, t=7时，'+kxo('s7','-')+' → 3a+(s+1)b',
					'特别地, t=9时，'+kxo('s9','-')+' → a+(s+1)b',
					]).join(br),
			...
			[ksc([kxo('st','-'),'其中(s,t)=1','t是两位数']).join(br),[
				ksc([
					'【割尾法2】*s = a*('+kxo('st','-')+'-t)+b*s → ta-sb',
					'特别地, s=1时，'+kxo('1t','-')+' → ta-b',
					'特别地, t=01时，'+kxo('s01','-')+' → a-sb',
					'特别地, t=03时，'+kxo('s03','-')+' → 3a-sb',
					'特别地, t=07时，'+kxo('s07','-')+' → 7a-sb',
					'特别地, t=09时，'+kxo('s09','-')+' → 9a-sb',
